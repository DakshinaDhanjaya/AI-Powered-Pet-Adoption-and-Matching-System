...
def load_model(filepath='pet_breed_model.pth', pretrained=True):
    ...
    Loads a saved model
    """
    # Load checkpoint with backward/forward compatibility
    checkpoint = torch.load(filepath, map_location=DEVICE)

    # Handle cases where the file is a state_dict or a dict with keys
    if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
        num_classes = checkpoint.get('num_classes', NUM_CLASSES)
        state = checkpoint['model_state_dict']
    elif isinstance(checkpoint, dict) and all(k.startswith('base_model') or k.startswith('features') or k.startswith('classifier') for k in checkpoint.keys()):
        # older checkpoint that stores state_dict directly with prefixed keys
        num_classes = NUM_CLASSES
        state = checkpoint
    else:
        # If it's neither, assume it's a raw state_dict
        num_classes = NUM_CLASSES
        state = checkpoint

    model = PetBreedModel(num_classes=num_classes, pretrained=False)
